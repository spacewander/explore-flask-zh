...
尽管Flask并不强迫你使用某个特定的模板语言，它还是默认你会使用Jinja。在Flask社区的大多数开发者使用Jinja，并且我建议你也跟着做。有一些插件允许你用其他模板语言进行替代，但除非你有充分理由（不懂Jinja可不是一个充分的理由），保持那个默认的选项；这样你会避免浪费很多时间来焦头烂额。

{ NOTE: Almost all resources imply Jinj2 when they refer to "Jinja." There was a Jinja1, but we be dealing with it here. When you see Jinja, we're talking about this: http://jinja.pocoo.org/ }// FIXME
...
模板的结构平行于路由的结构。对应于路由myapp.com/admin/analytics的模板是*templates/admin/analytics.html*。这里也有一些额外的模板不会被独立渲染。*layout.html*文件就是用于被其他模板继承的。
...
继承是通过`{% extends %}`和`{% block %}`标签。在双亲模板中，你可以定义要给子模板处理的block。
...
现在我们调用了一个未定义的宏 - `nav_link` - 并传递两个参数给它：一个目标（比如目标视图的函数名）和我们想要展示的文本。
...
{ NOTE: `from x import y`语句中要求x是相对于y的路径。如果我们的模板位于*myapp/templates/user/blog.html*，我们需要使用`from "../macros.html" import nav_link with context`。
}
