...
然而当你不得不管理一个生产上的应用的配置时，这一切变得棘手万分。
...
有一些配置变量是内建的，比如`DEBUG`。还有些配置变量是关于Flask插件的，比如`BCPYRT_LEVEL`就是用于Flask-Bcrypt插件（一个用于hash映射密码的插件）。
...
instance文件夹是根目录的一个子文件夹，包括了一个特定于当前应用实例的配置文件。它不会被提交到版本控制中。
